...
```
$ node scraper.js                  # coleta de chats ao vivo da Twitch (tmi.js)
$ python filter_and_merge.py       # limpeza, deduplicação e consolidação -> final_chat.csv
$ python annotate.py               # rotulação automática (léxico + regras) -> final_chat_labeled.csv
$ python export_toxic.py           # separa mensagens tóxicas / não tóxicas
$ python export_girias.py          # exporta ocorrências de gírias
$ python toxicity_by_stream.py     # taxa de toxicidade por canal
$ python check_duplicates.py       # auditoria de duplicatas
$ python gerar_graficos.py         # gera os gráficos usados no TCC
```
...
* *Pipeline* de limpeza (`filter_and_merge.py`)
   ...
   * Consolidação de todo o conteúdo em um único *dataset* (`final_chat.csv`)
...
* Rotulação automática (`annotate.py`)
...
* Exportações derivadas
   * `export_toxic.py`: separação em mensagens tóxicas e não tóxicas
   * `export_girias.py`: extração de ocorrências de gírias identificadas
   * `toxicity_by_stream.py`: taxa de toxicidade por canal monitorado
* Auditoria de qualidade (`check_duplicates.py`)
   * Validação da integridade do processo de deduplicação sobre os artefatos exportados
* Visualizações (`gerar_graficos.py`)
